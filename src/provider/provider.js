...
provider.link.DEFAULT_PROVIDER = {
  ...
  getDistance: function (link) {
    if (link.type === graph.link.LinkTypes.VALUE) {
      return (
        (13 / 8) *
        (provider.node.getSize(link.source) +
          provider.node.getSize(link.target))
      );
    } else {
      return (
        (20 / 8) *
 ...
 */
provider.node.DisplayTypes = Object.freeze({
  TEXT: 0,
  IMAGE: 1,
  SVG: 2,
  SYMBOL: 3,
...
provider.node.getProvider = function (label) {
  if (label === undefined) {
    ...
    if (provider.node.Provider.hasOwnProperty(label)) {
        ...
        if (provider.node.Provider.hasOwnProperty(p)) {
          ...
          if (nProvider.hasOwnProperty("children")) {
            if (nProvider["children"].indexOf(label) > -1) {
              logger.debug(
                "No provider is defined for label (" +
                  label +
                  "), parent (" +
                  p +
                  ") will be used"
      ...
      logger.debug(
        "No label provider defined for label (" +
          label +
          ") default one will be created from provider.node.DEFAULT_PROVIDER"
      );
      ...
      for (var prop in provider.node.DEFAULT_PROVIDER) {
        if (provider.node.DEFAULT_PROVIDER.hasOwnProperty(prop)) {
          provider.node.Provider[label][prop] =
            provider.node.DEFAULT_PROVIDER[prop];
        }
      }
      return provider.node.Provider[label];
    }
  ...
  if (!nProvider.hasOwnProperty(name)) {
    ...
    if (!isPropertyFound) {
      logger.debug(
        'No "' +
          name +
          '" property found for node label provider (' +
          label +
      ...
      if (provider.node.DEFAULT_PROVIDER.hasOwnProperty(name)) {
        ...
        logger.debug(
          'No default value for "' +
            name +
            '" property found for label provider (' +
            label +
            ")"
        );
      }
...
provider.node.getReturnAttributes = function (label) {
  ...
  if (provider.node.DEFAULT_PROVIDER.hasOwnProperty("returnAttributes")) {
    for (
      var k = 0;
      k < provider.node.DEFAULT_PROVIDER.returnAttributes.length;
      k++
    ) {
      if (
        provider.node.DEFAULT_PROVIDER.returnAttributes[k] !==
        query.NEO4J_INTERNAL_ID
      ) {
...
provider.node.filterNodeValueQuery = function (node, initialQuery) {
  return provider.node.getProperty(node.label, "filterNodeValueQuery")(
    node,
    initialQuery
  );
};
...
provider.node.filterNodeCountQuery = function (node, initialQuery) {
  return provider.node.getProperty(node.label, "filterNodeCountQuery")(
    node,
    initialQuery
  );
};
...
provider.node.filterNodeRelationQuery = function (node, initialQuery) {
  return provider.node.getProperty(node.label, "filterNodeRelationQuery")(
    node,
    initialQuery
  );
};
...
provider.node.getGenerateNegativeNodeValueConstraints = function (node) {
  return provider.node.getProperty(
    node.label,
...
provider.node.DEFAULT_PROVIDER = {
  ...
  displayResults: function (pElmt) {
    ...
    returnAttributes.forEach(function (attribute) {
      var div = pElmt.append("td").attr("class", "ppt-result-attribute-div");
      var attributeName = attribute;
      console.log("log from sam");
